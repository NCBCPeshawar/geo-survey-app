...
class ViewOnMapPage extends StatefulWidget {
  User user;
  ViewOnMapPage(this.user);

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _ViewOnMapPageState();
  }
}
...
class _ViewOnMapPageState extends State<ViewOnMapPage> {
  ...
  void onViewPolygonsClick({bool export = false}) {
    ...
    myUserRef
      ...
      if (export) writeToFile(categoryDataModel.polygons);
    });
  }
  ...
  Future<void> createPolygons(List<PolygonDataModelMap> polygons) async {
    ...
    if (polygonId == 0) {
      ...
      // if there is atleast 1 good polygon
  ...
  Future<void> writeToFile(List<PolygonDataModelMap> data) async {
    List<PolygonDataModel> pdm = [];
    final file = await _localFile;

    data.forEach((p) {
      pdm.add(PolygonDataModel(
          area: p.value.area,
          name: p.value.name,
    ...
    List jsonList = PolygonDataModel.encodeToJson(pdm);
    print('saving data');
    await file.writeAsString(jsonEncode(jsonList));
    print('data saved');
    Share.file(
        "$selectedSurvey-$selectedCategory-polygons",
        "$selectedSurvey-$selectedCategory-polygons.json",
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ...
                          Text("Area: " +
                              AreaUnitUtils()
                                  .getAreaAsString(area, areaUnitPref)),
                          InkWell(
                            child: Icon(
                              Icons.delete,
