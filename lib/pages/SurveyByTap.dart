...
class SurveyByTap extends StatefulWidget {
  User user;
  SurveyByTap(this.user);

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _SurveyByTapState();
  }
}

class _SurveyByTapState extends State<SurveyByTap> {
  ...
  void handleTimeout() async {
    // if (_currentPosition == null || (!_getting_location)) {
    //   //show alert box and exit app
    //   showExitDialog("Cannot confirm location",
    //       "App can work only when GPS is working fine. Exiting App");
    // }
  }
  ...
  Future getMyPositionStream() async {
    ...
    _streamPosition.listen((Position position) async {
      setState(() {
        _currentPosition = position;
      });
    ...
    currentCatRef.child("polygons").child(nextKey).set(<String, dynamic>{
      ...
      setState(() {
        _setPolygons.add(new Polygon(
          ...
          onTap: () {
            currentCatRef.child('polygons').child(nextKey).get().then(
              (DataSnapshot snapshot) {
                var polyData = snapshot.value as Map;
                setState(() {
                  showDataBox = true;
                  documentId = nextKey;
                  polyFieldName = polyData['name'].toString();
  ...
  void startProcesses() async {
    ...
    getFirebaseData();
    setState(() {
      selectedSurvey = "mysurvey";
      selectedCategory = "random";
    });

    await getMyPositionStream();
